...
public class ShardedRedis implements JedisInterface {
	...
	private void initialShardedPool(Properties properties) {
		...
		boolean testOnBorrow = Boolean
				.parseBoolean(properties.getProperty("redis.ShardedJedisPoolConfig.testOnBorrow"));
		// 权限验证auth
		...
		for (String redisUrl : redisUrls.split(DEFAULT_REDIS_SEPARATOR)) {
			...
			JedisShardInfo ShardedJedisinfo = new JedisShardInfo(redisUrlInfo[0], Integer.parseInt(redisUrlInfo[1]),
					timeout);
	 ...
	 * </p>
	 * <p>
	 * Description: 返回byte数组
	 * </p>
	 * 
	 * @param key
	 * @return
	 * @see com.troila.redis.JedisInterface#getByte(java.lang.Byte[])
	 */
	@Override
	public byte[] getByte(final byte[] key) {
		// TODO Auto-generated method stub
		return execute(new String(key), new ShardedRedisExecutor<byte[]>() {
			@Override
